...
func NewHandler(cfg Config, opts ...HandlerOption) (*RewritingHandler, error) {
	...
	h.GET("/webapi/mfadevice/:token", httplib.MakeHandler(h.getMFADevicesWithTokenHandle))
	h.DELETE("/webapi/mfadevice", httplib.MakeHandler(h.deleteMFADeviceWithTokenHandle))
	h.POST("/webapi/mfadevice", h.WithAuth(h.addMFADeviceWithTokenHandle))
	h.GET("/webapi/mfadevice", h.WithAuth(h.getMFADevicesHandle))
...
func (h *Handler) changePasswordWithToken(w http.ResponseWriter, r *http.Request, p httprouter.Params) (interface{}, error) {
	...
	if _, err := h.newSessionResponse(ctx); err != nil {
		return nil, trace.Wrap(err)
	}
